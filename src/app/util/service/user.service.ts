...
export class UserService {
	 ...
	 */
	private async handleGoogleSignInResult(result: UserCredential): Promise<void> {
	 ...
	 */
	private async handleExistingGoogleUser(firebaseUser: any, userSnap: any): Promise<void> {
		console.log("✅ User already exists in Firestore");

		const userData = userSnap.data();
		localStorage.setItem(`user-data-${firebaseUser.uid}`, JSON.stringify(userData));
	}
	...
	private handleGoogleSignInError(error: unknown): void {
		...
		switch (authError.code) {
			...
			case "auth/cancelled-popup-request":
				...
				break;
			default:
				console.error("❌ Unexpected error during Google sign-in:", authError.message);
		}
	}
	 ...
	 */
	private async createUserInFirestore(uid: string, userData: User): Promise<void> {
		const userRef = doc(this.firestore, `users/${uid}`);
	 ...
	 */
	private mapBankAccountType(bankAccountType: "checking" | "savings" | "credit" | "investment"): "bank" | "cash" | "credit" | "loan" {
